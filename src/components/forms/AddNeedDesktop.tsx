...
import { NeedTagType, TicketFormData } from "../../services/ticket.type"

type AddNeedDesktopProps = {
  typeTags: NeedTagType[]
  toggleTypeTag: (id: number) => void
  selectedTypeTags: number[]
  locationTags: {
    value: number
    label: string
  }[]
  register: (TicketFormData) => any
  addTicketMutation: any
  setWhereFromTag: (tagId: number) => void
  setWhereToTag: (tagId: number) => void
}

export const AddNeedDesktop = (props: AddNeedDesktopProps) => {
  const {
    typeTags,
    toggleTypeTag,
    selectedTypeTags,
  ...
  return (
    <div className="hidden md:flex">
      <Stack>
        ...
        <div className="h-4 border-solid"/>
